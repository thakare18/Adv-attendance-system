    ...
    while True:
        ...
        for (x, y, w, h) in faces:
            ...
            if (conf < 50):
                ...
                matched_row = df.loc[df['SERIAL NO.'] == serial]
                if not matched_row.empty:
                    bb = str(matched_row.iloc[0]['NAME']).strip()
                    ID = str(matched_row.iloc[0]['ID']).strip()
                else:
                    bb = 'Unknown'
                    ID = 'Unknown'
                ...
                Id = 'Unknown'
                bb = str(Id)
            cv2.putText(im, str(bb), (x, max(y - 10, 20)), font, 1, (255, 255, 255), 2)
